...
bot.on('message', async (msg) => {
	...
	else if (command === './add')
	{
		...
		{
			if(msg.author.id === configuration.owner)
			{
	...
	else if (command === './about')
	{
		...
		.setTitle('About')
		.addFields(
			{ name: 'Developer : ', value: ',`MarOッ,#1211'},
			{ name: 'Language : ', value: 'Javascript'},
			{ name: 'Website : ', value: 'SOON'},
		)
		.setTimestamp()
		.setFooter('Contact : ,`MarOッ,#1211');
		...
		if(!req)
		{
			msg.channel.send(`> Contact ,\`MarOッ,#1211 for access ;)`);
		}
